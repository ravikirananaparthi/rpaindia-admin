...
const UpdateMembershipRequestForm = ({ request, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Membership request updated successfully!");
      onUpdate();
      onClose();
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-6 text-gray-900">Update Membership Request</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          {[
            { id: 'fullName', label: 'Full Name', type: 'text' },
            { id: 'contactNumber', label: 'Contact Number', type: 'tel' },
            { id: 'location', label: 'Location', type: 'text' },
            { id: 'membershipType', label: 'Membership Type', type: 'text' }
          ].map((field) => (
            <div key={field.id}>
...
export default UpdateMembershipRequestForm;